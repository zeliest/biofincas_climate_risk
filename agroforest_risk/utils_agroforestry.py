...
with open("species_thresholds.json", "r") as f:
    species_thresholds = json.load(f)

def make_threshold_impf(species: str, impf_id: int, zero: bool = False) -> ImpactFunc:
    """Impact function: 0 impact until threshold, then 1 impact after (or flat zero if dummy)."""
    thr = species_thresholds.get(species, 0.5)  # fallback if missing
    
    impf = ImpactFunc()
    impf.haz_type = species
    impf.id = impf_id
    impf.name = f"{'Zero' if zero else 'Step'} impact for {species}"

    if zero:
        # Completely flat zero line
        impf.intensity = np.array([0, 1])
        impf.mdd = np.array([0, 0])
        impf.paa = np.array([1, 1])
    else:
        # Step curve using threshold
        impf.intensity = np.array([0, thr, thr + 0.01, 1])
        impf.mdd = np.array([1, 1, 0, 0])
        impf.paa = np.array([1, 1, 1, 1])
...
def get_target_species(excel_files, species_replacements):
    """Scan Excel files and build a harmonised global species list."""
    all_species = set()
    for file_path in excel_files:
        ...
        for sheet in xls.sheet_names:
            ...
            if "Scientific name" not in df.columns:
                continue
            species_list = df.loc[df["Species"] != "Coffee (yield)", 
                                  "Scientific name"].dropna().tolist()
            all_species.update(
                species_replacements.get(s.strip(), s.strip())
                for s in species_list
            )
    # Ensure crops are always in the list
    all_species.update(["Coffea arabica", "Theobroma cacao"])
    return sorted(all_species)


def build_exposures_from_sheet(df, target_species, species_replacements,
                               vary_counts: bool = True,
                               dirichlet_strength: float = 10.0) -> Exposures:
    """Convert one Excel sheet to a CLIMADA Exposures object."""
    df = df.copy()
    df["species_clean"] = df["Scientific name"].apply(
        lambda s: species_replacements.get(s.strip(), s.strip())
    )
    geometry = [Point(xy) for xy in zip(df["Longitude"], df["Latitude"])]
    gdf = gpd.GeoDataFrame(df, geometry=geometry, crs="EPSG:4326")

    # tree counts with uncertainty
    if vary_counts:
        total_trees = gdf["Plants/ha"].sum()
        if total_trees <= 0 or np.isnan(total_trees):
            gdf["value"] = gdf["Plants/ha"].fillna(0).astype(int)
        else:
            alpha = (gdf["Plants/ha"].values / total_trees) * dirichlet_strength
            # avoid invalid alphas
            alpha = np.where((alpha <= 0) | np.isnan(alpha), 1e-6, alpha)
            proportions = np.random.dirichlet(alpha)
            proportions = np.nan_to_num(proportions, nan=0.0)
            gdf["value"] = np.round(proportions * total_trees).astype(int)
    else:
        gdf["value"] = gdf["Plants/ha"].fillna(0).astype(int)

    # shading
    gdf["Shade_raw"] = gdf["Per-tree shading (%)"] * gdf["value"]
    gdf["Shade contribution (%)"] = (
        gdf.groupby(["Latitude", "Longitude"])["Shade_raw"]
           .transform(lambda x: 100 * x / x.sum())
    )

    # impf_<species> columns
    for species in target_species:
        colname = f"impf_{species}"
        gdf[colname] = (gdf["species_clean"] == species).astype(float) + 1

    return Exposures(gdf)


def build_exposures_from_file(file_path, target_species, species_replacements,
                              vary_counts=True, dirichlet_strength=10):
    """Loop over all sheets in one Excel file and return exposures dict."""
    exposures = {}
    xls = pd.ExcelFile(file_path)
    for sheet in xls.sheet_names:
        df_sheet = pd.read_excel(file_path, sheet_name=sheet)
        if "Scientific name" not in df_sheet.columns:
            continue
        exposures[sheet] = build_exposures_from_sheet(
            df_sheet, target_species, species_replacements,
            vary_counts=vary_counts, dirichlet_strength=dirichlet_strength
        )
    return exposures
